...
const API_URL = 'http://www.omdbapi.com/?i=tt3896198&apikey=6df46fe0';
...
function getMovieTemplate(movie, cols, button = true) {
  ...
  <div class="card col-${cols}">
    ...
    <div class="card-body">
      ...
      ${
        button ?
        `<button data-id="${movie.imdbID}" type="button" class="btn btn-primary btn-danger watch-later-button">Watch Later</button>`
        : ''
      }
    </div>
  </div>
...
function showError(error) {
  resultsSection.innerHTML = `
    <div class="alert alert-danger" class="col" role="alert">
    ${error.message}
    </div>
  `;
}